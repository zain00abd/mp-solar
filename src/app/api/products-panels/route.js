...
export async function GET(request) {
  try {
    ...
    const stats = await Panel.aggregate([
      { $match: filter },
...
export async function POST(request) {
  try {
    ...
    const duplicate = await Panel.findOne({ name: { $regex: new RegExp(`^${name}$`, 'i') }, company });
    if (duplicate) {
      return NextResponse.json({ success: false, error: 'Panel with this name already exists for this company' }, { status: 409 });
    }

    const panel = new Panel({ name, company, image, pdfUrl, description, features: features || [], models: models || [], specs: specs || [], tags: tags || [], warranty: warranty || {}, category: 'products' });
    await panel.save();
